...
static PerimeterExtrusions extract_ordered_perimeter_extrusions(const PerimeterExtrusions &sorted_perimeter_extrusions, const bool external_perimeters_first) {
    ...
    // ORCA:
    // After the main loop, check for any unvisited perimeters and add them to the ordered list
    // This fixes an edge case where on rare occasions the last perimeter is not visited in the stack traversal above
    for (size_t i = 0; i < sorted_perimeter_extrusions.size(); ++i) {
        if (!visited[i]) {
            if (external_perimeters_first)
                ordered_extrusions.emplace_back(sorted_perimeter_extrusions[i]);
            else
                ordered_extrusions.insert(ordered_extrusions.begin(), sorted_perimeter_extrusions[i]);
        }
    }
    // END ORCA
